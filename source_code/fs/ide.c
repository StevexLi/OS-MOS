...
void ide_read(u_int diskno, u_int secno, void *dst, u_int nsecs) {
	...
	for (u_int off = 0; begin + off < end; off += BY2SECT) {
		...
		temp = begin + off;
		panic_on(syscall_write_dev((u_int)(&temp), DEV_DISK_ADDRESS + DEV_DISK_OFFSET, 4));
		temp = DEV_DISK_OPERATION_READ;
		panic_on(syscall_write_dev((u_int)(&temp), DEV_DISK_ADDRESS + DEV_DISK_START_OPERATION, 4));
		panic_on(syscall_read_dev((u_int)(&temp), DEV_DISK_ADDRESS + DEV_DISK_STATUS, 4));
		if (!temp)
			user_panic("ide_read failed");
		panic_on(syscall_read_dev((u_int)(dst+off), DEV_DISK_ADDRESS + DEV_DISK_BUFFER, DEV_DISK_BUFFER_LEN));
	}
}
...
void ide_write(u_int diskno, u_int secno, void *src, u_int nsecs) {
	...
	for (u_int off = 0; begin + off < end; off += BY2SECT) {
		...
		temp = begin + off;
		panic_on(syscall_write_dev((u_int)(&temp), DEV_DISK_ADDRESS + DEV_DISK_OFFSET, 4));
		panic_on(syscall_write_dev((u_int)(src+off), DEV_DISK_ADDRESS + DEV_DISK_BUFFER, DEV_DISK_BUFFER_LEN));
