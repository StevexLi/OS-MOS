...
void mips_detect_memory() {
	...
	npage = memsize/BY2PG; // fixed page size of 4 Kbytes
...
int page_alloc(struct Page **new) {
	...
	memset(page2kva(pp),0,BY2PG); // page->page num->page frame addr(pa)->KADDR
	*new = pp;
	return 0;
}
...
static int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte) {
	...
	int flag_pte_exist = 1;
	if ((!((*pgdir_entryp) & PTE_V)) && create) {
		if (page_alloc(&pp) == (-E_NO_MEM))
			return -E_NO_MEM;
		*pgdir_entryp = (PTE_ADDR(page2pa(pp)))|PTE_D|PTE_V;
		pp->pp_ref += 1;
		flag_pte_exist = 0;
	}
	...
	if ((!flag_pte_exist) && (!create)) {
		*ppte = NULL;
	} else {
		*ppte = (Pte *)(KADDR(PTE_ADDR(*pgdir_entryp))) + PTX(va); //pointer to second level pte
	}
	return 0;
}
