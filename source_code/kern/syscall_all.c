...
int sys_mem_map(u_int srcid, u_int srcva, u_int dstid, u_int dstva, u_int perm) {
	...
	Pte* temp_pte;
	if ((pp = page_lookup(srcenv->env_pgdir, srcva, &temp_pte)) == NULL) {
		return -E_INVAL;
	}
...
int sys_set_env_status(u_int envid, u_int status) {
	...
	if (env->env_status != status) {
		if (status == ENV_RUNNABLE) { // previously not runnable
			TAILQ_INSERT_TAIL(&env_sched_list, env, env_sched_link);
		}
		else { // previously runnable
...
int sys_ipc_try_send(u_int envid, u_int value, u_int srcva, u_int perm) {
	...
	if (srcva != 0) {
		...
		Pte* temp_pte;
		if ((p = page_lookup(curenv->env_pgdir, srcva, &temp_pte)) == NULL) {
			return -E_INVAL;
		}
...
int sys_write_dev(u_int va, u_int pa, u_int len) {
	...
	if ((pa>=0x10000000 && pa+len<=0x10000000+0x20) || (pa>=0x13000000 && pa+len<=0x13000000+0x4200) || (pa>=0x15000000 && pa+len<=0x15000000+0x200)) {
