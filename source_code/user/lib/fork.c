...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
	...
	memcpy((void *)UCOW, ROUNDDOWN(va, BY2PG), BY2PG);
...
static void duppage(u_int envid, u_int vpn) {
	...
	addr = vpn << PGSHIFT;
	perm = vpt[vpn] & 0xFFF;
	// addr = PTE_ADDR(*(vpt + vpn));
	// perm = (*(vpt + vpn)) & 0xFFF;
	// addr = PTE_ADDR(*(curenv->env_pgdir + vpn));
	// perm = (*(curenv->env_pgdir + vpn)) & 0xFFF;
	...
	if ((perm & PTE_LIBRARY) || !(perm & PTE_D)) { //shared page or readonly page
		syscall_mem_map(0, (void *)addr, envid, (void *)addr, perm);
	} else if (perm & PTE_COW) { // COW page from parent's parent
		syscall_mem_map(0, (void *)addr, envid, (void *)addr, (perm|PTE_COW)&(~PTE_D)); // exactly same?
	} else { // with PTE_D but not COW page
		syscall_mem_map(0, (void *)addr, envid, (void *)addr, (perm|PTE_COW)&(~PTE_D)); // child
		syscall_mem_map(0, (void *)addr, 0, (void *)addr, (perm|PTE_COW)&(~PTE_D)); // parent
	}
}
